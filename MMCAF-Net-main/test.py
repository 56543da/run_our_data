...
def test(args,table):
    ...
    #    print(name)
    #raise ValueError("fused_preds是{}".format(fused_preds))
    print ("Stage 2")
    ...
    with tqdm(total=len(data_loader.dataset), unit=' windows') as progress_bar:
        for i, (inputs, targets_dict) in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                cls_targets=targets_dict['is_abnormal'].to(args.device)

                #w 输入输出
                #w loss,ip_out,sh_out,tp_out=model(inputs,table_data,cls_targets)

                #out = model(inputs, tab,targets_dict['bbox'],targets_dict['is_abnormal'])
                out = model(inputs,tab)
                cls_logits=out
...
if __name__ == '__main__':
    ...
    if not os.path.exists(csv_path):
        print(f"Warning: Metadata file not found at {csv_path}. Using empty table.")
        table_data = pd.DataFrame(columns=['NewPatientID', 'Sex', 'Age', 'Weight', 'T-Stage', 'N-Stage', 'M-Stage', 'Smoking'])
    else:
