...
class ModelEvaluator(object):
    ...
    def evaluate(self, model, device, epoch=None, table=None):
        ...
        with tqdm(total=num_examples, unit=' ' + phase) as progress_bar:
            ...
            for img, targets_dict in data_loader:
                ...
                with torch.no_grad():
                    ...
                    #w forward
                    #w out, loss_out, others = model(img, tab, label)
                    #w if loss_out == None:
                    #w     loss = self.cls_loss_fn(out, label).mean()
                    #w else:
                    #w     loss_cls = self.cls_loss_fn(out, label).mean()
                    #w     loss = (loss_out + loss_cls) / 2
                    #w out = model(tab)
                    #w loss = self.cls_loss_fn(out, label).mean()
                    ...
                    #w forward StageOne2
                    '''loss0, i_sp_out, i_sh_out, t_sp_out, t_sh_out = model(img, tab, label)
                    loss1 = self.cls_loss_fn(i_sp_out, label).mean()
                    loss2 = self.cls_loss_fn(i_sh_out, label).mean()
                    loss3 = self.cls_loss_fn(t_sp_out, label).mean()
                    loss4 = self.cls_loss_fn(t_sh_out, label).mean()
                    loss = loss0 + loss1 + loss2 + loss3 + loss4'''



                    #w forward StageOne3
                    '''intra_closs, inter_closs, sample_level_closs, loss_sp, loss_sh = model(img, tab, label)
                    loss = intra_closs + inter_closs + sample_level_closs + loss_sp + loss_sh'''
                    '''loss_sp, loss_sh = model(img, tab, label)
                    loss = loss_sp + loss_sh'''

                    #w forward StageOne3_Img
                    '''out, others = model(img, tab, label)
        ...
        if probs is not None:
            ...
            try:
                ...
                curves.update({
                    ...
                    phase + '_' + 'CM': sk_metrics.confusion_matrix(labels, preds)
                })
            except ValueError:
                pass
