...
class MMCAF_Net(nn.Module):
    def __init__(self, **kwargs):
        ...
        # 获取动态特征数量，默认为 7
        num_tab_features = kwargs.get('num_tab_features', 7)
        print(f"MMCAF_Net initializing with {num_tab_features} tabular features.")
        self.kan = KANLayer(in_dim=num_tab_features, out_dim=24).to(self.device)   #设置输入维度和输出维度
